...
PROJECT_NAME=erico-training
STACK_NAME=erico-training-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://erico-training.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t iRohitSingh/erico-training-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t iRohitSingh/erico-training-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name erico-training-frontend-acceptance --link erico-training-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d iRohitSingh/erico-training-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name erico-training-backend-acceptance -d iRohitSingh/erico-training-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop erico-training-frontend-acceptance
	@docker stop erico-training-backend-acceptance
